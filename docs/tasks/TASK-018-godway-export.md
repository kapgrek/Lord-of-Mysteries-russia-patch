...
- **Под вопросом для чата анализа (исходная пометка):** если UI-материалы и в пробе 3 не рисуются в RT, шаг 3 плана (кадры материалов через `DrawMaterialToRenderTarget`) невыполним в текущем виде.
