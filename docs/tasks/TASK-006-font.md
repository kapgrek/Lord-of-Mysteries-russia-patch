...
5. **Кириллица в Title — пропорциональная.** Целевой механизм — `"subfont"`: он **дополняет** шрифт игры, а не заменяет его, поэтому переживает обновления игры (изменения `Font_Aleo` от разработчиков сохраняются) и позволяет позже подключить собственный шрифт с засечками (этап 4b) без переделки. Режим задаётся константой `CYRILLIC_FONT_MODE` в `Init.lua` (для релиза) и полем `CyrillicFont` в `absoluteru_dev.lua` (перекрывает константу для тестов). Значения: `"subfont"`, `"typeface"`, `"off"`. **В релизной константе v2.9.2 — `"typeface"`**; после того как пользователь подтвердит `"subfont"` в игре (чек-лист, п. 6), константа переключается на `"subfont"` одной строкой.
   - **Список face по приоритету** — таблица `runtimeFixes.CyrillicFaceCandidates` в начале блока шрифтов: `{ "/Game/AbsoluteRU/Fonts/<будущий шрифт этапа 4b>" (пока закомментирован), <face из Regular-typeface Font_Aleo, найденный пробой шага 1> }`. Первый, который загрузился через `slua.loadObject`, становится источником кириллицы. Путь face из пробы не хардкодить вслепую: брать его в рантайме из `Font_Aleo.CompositeFont.DefaultTypeface.Fonts` (запись `Regular`); если его там нет (кириллица Regular идёт из `FallbackTypeface`) — из `FallbackTypeface.Typeface.Fonts[1]`.
   - `"subfont"`: один раз при старте `Init.lua` (до первых ремонтов UI) добавить в `Font_Aleo.CompositeFont.SubTypefaces` запись `{CharacterRanges = [0x0400–0x045F, 0x2116], Typeface.Fonts = [{Name="Title", face}, {Name="Regular", face}]}` (имена typeface — те, что есть в `DefaultTypeface`), записать структуру обратно (`font.CompositeFont = cf`), прочитать обратно и убедиться, что запись в `SubTypefaces` есть, затем вызвать `C7FunctionLibrary.FlushFontCache()`. Повторный запуск не должен дублировать запись (проверять по диапазону). Каждый шаг — в `pcall`; итог одной строкой: `[CPDDRuntimeFix] cyrillic font mode=subfont face=<путь> write=<ok|err> verify=<ok|fail> flush=<ok|missing|err>`. Любая неудача → автоматический откат в `"typeface"` и строка `mode=typeface reason=<…>`.
   - `"typeface"` (запасной): в `translateTextWidget`, если в тексте есть кириллица, шрифт `Font_Aleo` и typeface `Title`, поставить typeface, который даёт пропорциональную кириллицу. По умолчанию `Regular`; если проба из шага 1 покажет Bold/Medium с тем же face, что у Regular, — его. В режиме `"subfont"` эта ветка не выполняется.
   - Rich text (`:3540-3559`) не трогать ни в одном режиме: там шрифт задаёт TextStyleSet, и правка ломает материалы (комментарий в коде). В режиме `"subfont"` RichText получит кириллицу из SubTypeface автоматически — это отдельный пункт проверки.
   - Одна строка в C7.log при старте (через `report`, `Log.Info`): `[CPDDRuntimeFix] cyrillic font mode=<…> …`.
...
7. **Этап 4b (мод-пак) в этом TASK не делать**, только записать в ROADMAP как отдельную исследовательскую задачу:
   ...
   - (b) при успехе — IoStore-контейнер **только с новым** `UFontFace` (Bitter / PT Serif) по собственному пути `/Game/AbsoluteRU/Fonts/…`; ассеты игры (`Font_Aleo` и др.) не заменять. Подключение — добавлением пути в начало `CyrillicFaceCandidates` (шаг 5); если пак не загрузился, Lua берёт следующий face. Нужен UE той же версии для кука UFontFace;
...
6. **Проверка целевого режима.** В `absoluteru_dev.lua` добавить `CyrillicFont = "subfont"`, перезапустить. В C7.log найти `cyrillic font mode=subfont face=… write=… verify=… flush=…` и прислать строку. Если там `mode=typeface reason=…` — прислать её. При `write=ok verify=ok flush=ok` пройти пп. 2–4 ещё раз; дополнительно — RichText (подсказка предмета, диалог): кириллица тоже пропорциональная, без квадратов. Если всё в порядке — сообщить в чат исполнения, чтобы переключить релизную константу на `"subfont"`.

## Промпт для чата исполнения
> Ты — чат исполнения проекта AbsoluteRU (AGENTS.md §3, чат 2). Прочитай AGENTS.md, docs/LESSONS.md, docs/DIAGNOSTICS.md, docs/tasks/ROADMAP.md и выполни docs/tasks/TASK-006-font.md, раздел «План», шаги 1–6 (шаг 7 только записать в ROADMAP). Главное из анализа: Title-typeface `Font_Aleo` рисует кириллицу китайским FZ Old Mincho шириной 1,0 em, Regular — Source Han Sans 0,62 em; наш `translateTextWidget` компенсирует это `LetterSpacing` −120/−60, а при предзагрузке ставит несуществующий typeface `"Font_Aleo"` (Init.lua:1577). Сделай: пробу CompositeFont в AbsruDiagnostics (только чтение, раздел `composite` в fonts.json + вывод в CollectDiagLogs), фиксированные StandardFontObject/CinematicFontObject вместо эвристик, узкую подмену шрифта только для не-Aleo с кириллицей, `LetterSpacing` 0 для кириллицы (без отрицательных). Целевой механизм — `"subfont"`: при старте дописать в `Font_Aleo.CompositeFont.SubTypefaces` диапазон U+0400–045F с face из списка `CyrillicFaceCandidates` (первый загрузившийся; пока это face Regular-typeface из самого `Font_Aleo`, позже — наш шрифт с засечками из этапа 4b), проверить запись чтением, вызвать `C7FunctionLibrary.FlushFontCache()`, при любой ошибке автоматически откатиться в `"typeface"` (Title → Regular для кириллицы). Шрифт игры не заменять, только дополнять. RichText не трогать. Релизная константа v2.9.2 = `"typeface"`, `"subfont"` включается через `absoluteru_dev.lua` до подтверждения пользователем. Пороги размера по длине не трогать (этап 5). Версия v2.9.2-RU. Проверь `tools/VerifyPatch.ps1`; `.ps1` с кириллицей сохраняй в UTF-8 с BOM, Lua — UTF-8 без BOM, LF. Не пиши «исправлено»: дай пользователю чек-лист из раздела «Проверка». Закоммить и запушь в main; релиз не публикуй без решения пользователя. В папку игры ничего не пиши и игру не запускай.
