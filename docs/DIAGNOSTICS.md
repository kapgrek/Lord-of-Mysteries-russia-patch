...
Одна сборка для всех. Без файла флагов разработчика патч работает как обычно: в `C7.log` две строки после старта, обе пишутся в `after_main` (`[CPDDRuntimeFix] cyrillic font mode=…` и `v2.9.5-RU active hooks_installed=N`), модуль диагностики не загружается, в горячих путях `Init.lua` добавлена одна проверка `runtimeFixes.Diag ~= nil`. С файлом флагов сборка собирает данные для следующих этапов:
...
return {
    ...
    -- CyrillicFont = "face",   -- режим кириллицы: "face" | "cultures" | "subfont" | "typeface" | "off"
    -- CyrillicTitleFace = "NotoSerif_Regular", -- для "face": чей face поставить в Title (по умолчанию "Aleo_Regular")
    -- CyrillicCultureSub = "NotoSans_Regular", -- для "cultures": какой SubTypeface взять первым
    -- CyrillicFlush = "culture", -- для "face"/"cultures": после FlushFontCache сменить культуру на en-US и обратно
}
```

Выключение: удалить `absoluteru_dev.lua`.

### Флаг `CyrillicFont` (TASK-006, TASK-008, TASK-009)
Перекрывает релизную константу `CYRILLIC_FONT_MODE` в `Init.lua` (в v2.9.5 — `"typeface"`). Читается, только если в файле `Enabled = true`. Это не наблюдение, а смена поведения, поэтому флаг по умолчанию закомментирован.
- `"face"` (TASK-009) — решающий тест: доходит ли рантайм-правка `CompositeFont` до Slate. В `Font_Aleo.CompositeFont.DefaultTypeface.Fonts` у записи с `Name == "Title"` заменяется `FontFaceAsset`; записи `*_SDF` не трогаются. Новый face задаёт флаг `CyrillicTitleFace`: `"Aleo_Regular"` (по умолчанию, face записи `Regular`) или имя SubTypeface `Font_Aleo` — `"NotoSerif_Regular"`, `"NotoSansCJKsc_Regular"`, `"NotoSans_Regular"` (первый face записи, у которой путь содержит `/Fallback/<имя>.`). Объект face берётся из уже прочитанной структуры, по пути не загружается. Запись через копию `CompositeFont` (`Fonts:Set`, иначе `Remove` + `Insert`), затем путь face у `Title` читается обратно, число записей сверяется, вызывается `C7FunctionLibrary.FlushFontCache()`. При любой ошибке прежний face возвращается и включается `"typeface"`. Подмена Title → Regular в этом режиме не выполняется: на экране должен быть виден эффект только самой правки шрифта.
- `"cultures"` (TASK-008) — диапазоны не читаются и не создаются. В `Font_Aleo.CompositeFont.SubTypefaces` ищется уже существующая запись, у которой первый face — `…/Fallback/<имя>.<имя>`; имена берутся из `runtimeFixes.CyrillicCultureSubs = { "NotoSerif_Regular", "NotoSans_Regular" }`, флаг `CyrillicCultureSub = "<имя>"` ставит своё имя первым. В её `Cultures` записывается текущая культура (`KismetInternationalizationLibrary.GetCurrentCulture()`, запасное `en`; для `en-US` — `en-US;en`), чтобы Slate считал её диапазоны приоритетными и проверял раньше default-typeface. Запись идёт через копию `CompositeFont` (`SubTypefaces:Set`, иначе `Remove` + `Insert`), затем `Cultures` читается обратно, число SubTypeface сверяется, вызывается `C7FunctionLibrary.FlushFontCache()`. При любой ошибке прежнее `Cultures` возвращается и включается `"typeface"`. Подмена Title → Regular в этом режиме не выполняется: кириллица Title должна прийти из выбранного sub. Изменение живёт только в памяти до выхода из игры.
...
**Когда применяется.** `face` и `cultures` пишутся при загрузке `Init.lua` (кадр 0, до экрана загрузки). Если `Font_Aleo` на кадре 0 ещё не загружен, попытка повторяется в `after_main` (хук 1500, перед выводом строки). В строке режима `applied_at=load|after_main` — когда запись прошла успешно.

**Флаг `CyrillicFlush = "culture"`** (TASK-009, по умолчанию выключен): после успешного `FlushFontCache` в режимах `face`/`cultures` культура переключается `KismetInternationalizationLibrary.SetCurrentCulture("en-US", false)` и обратно на исходную (если исходная `en-US` — через `en`). В UE смена культуры сбрасывает кэши шрифтов Slate. Результат — `flush2=ok|err:<…>` в строке режима; без флага поля нет.

Итог — одна строка в C7.log:
- `face`: `[CPDDRuntimeFix] cyrillic font mode=face source=<CyrillicTitleFace> title_face=<путь нового face> previous=<путь прежнего> write=<ok(set)|ok(insert)|err> verify=<ok|fail> flush=<ok|missing|err> [flush2=…] applied_at=<load|after_main>`; при откате `reason` = `no_title`, `title_face_unreadable`, `no_face:<имя>`, `write:<ошибка>`, `verify`, `verify_count`, `verify_no_title`, `flush`;
- `subfont`: `[CPDDRuntimeFix] cyrillic font mode=subfont face=<путь> write=<ok|exists|err> verify=<ok|fail> flush=<ok|missing|err>`;
- `cultures`: `[CPDDRuntimeFix] cyrillic font mode=cultures sub=<путь face> cultures=<…> previous=<прежнее> write=<ok(set)|ok(insert)|err> verify=<ok|fail> flush=<ok|missing|err> [flush2=…] cyr=<yes|no|unknown> latin=<yes|no|unknown> applied_at=<load|after_main>` (`cyr`/`latin` — покрывает ли sub U+0410 / U+0041; `unknown`, пока диапазоны не читаются, см. пробу `.get`);
- при откате `mode=typeface reason=<…> … title_typeface=Regular` (`reason` у `cultures`: `no_sub`, `cultures_unreadable`, `write:<ошибка>`, `verify`, `verify_count`, `verify_no_sub`, `flush`); в релизе `mode=typeface title_typeface=Regular`;
- `Font_Aleo` не загрузился: `mode=off reason=Font_Aleo_not_loaded requested=<режим> stage=<load|after_main>` (для `face`/`cultures` строка со `stage=load` заменяется результатом повтора в `after_main`).

**`cultures` в v2.9.4 на экране ничего не изменил** (TASK-009): оба теста дали `write=ok(set) verify=ok flush=ok`, в том числе на `NotoSansCJKsc_Regular`, который кириллицу точно покрывает. Режим `face` различает две гипотезы: правка не доходит до Slate (H1) или доходит, но приоритет `Cultures` не перебивает default-face (H2).

То же пишется в `fonts.json → cyrillic_font`. Режим выбирается при загрузке `Init.lua`, но строка выводится в `after_main`, прямо перед `active hooks_installed=`: `Log.Info` до инициализации логгера игры в C7.log не попадает (TASK-007).

`subfont` в v2.9.3–v2.9.4 откатывается: `FInt32Range` в slua непрозрачен (сессия 2026-09-25_1046: у метатаблицы `index_keys=[]`, методы — `attempt to call method`, `LowerBound` — `nil`), поэтому `cyrillicSubIndex` не находит диапазон, а `newRange` пишет в `nil`. Режим не чинится, пока не ясно, нужен ли он (TASK-008). Папку `Saved\Mods\logs\` можно удалить вручную (Lua удалять файлы не умеет).
...
| `absru-s<n>-session.json` | `sid`, `slot`, `version`, `started`, `last_flush`, `flags`, `dir`, `parts`, `lines`, `api` (какие API сработали: `GetParent`, `GetClass`, `Brush.ResourceObject`, `GetDesiredSize`, `GetCachedGeometry`, `IsVisible`, `GetPathName`, `UFont.CompositeFont`, `C7FunctionLibrary`; `C7FunctionLibrary.FlushFontCache` — Lua-тип поля, функция не вызывается; `FInt32Range.LowerBound`, `FInt32Range.LowerBound.Value`, `FInt32Range.GetLowerBoundValue`, `FInt32Range.Contains` — результат на первом диапазоне `Font_Aleo`; `import(Int32Range)`, `import(CompositeSubFont)`, `import(TypefaceEntry)` — `import=ok|fail ctor=<тип>|fail`, новая структура только читается и отбрасывается; `culture.GetCurrentCulture` / `GetCurrentLanguage` / `GetCurrentLocale` — из `KismetInternationalizationLibrary`), `gauges` (от `bootstrap.lua`), `budget` (`ticks`, `tick_ms_max`, `max_item_ms`, `max_item_kind`, `io_ms_max`, `flush_ms_max`, `queue_peak`, `flushes`), `dropped`, `errors`, `counters`, `pending`, `metrics` (весь `runtimeMetrics`) |
...
| `absru-s<n>-fonts.json` | `standard`, `cinematic`, `cyrillic_font` (`mode`, `requested`, `title_typeface`, `typefaces`, `face`, `sub`, `cultures`, `previous`, `write`, `via`, `verify`, `flush`, `cyr`, `latin`, `reason`; для `face` — `source`, `title_face`; `flush2`, `applied_at`), `composite` (`<путь>` → `loaded`, `error`, `default[]` и `fallback.fonts[]` (`name`, `face`, `loading`, `hinting`, `subface`), `fallback.scaling`, `subs[]` (`cultures`, `scaling`, `ranges[]` (`low`, `high`, `low_type`, `high_type`, `read` = `field`/`method`/`get` (геттеры из таблицы `.get` метатаблицы, TASK-008); если slua не отдал `FInt32Range` — `raw`, `type`), `covers_0410`, `covers_0041` (покрывает ли sub кириллическую А / латинскую A; поля нет — диапазоны не прочитаны), `range_read` (`field`, `method`, `get`, `contains`, `none`), `range_probe` (только `Font_Aleo`, первый диапазон: `meta`, `meta_name`, `meta_keys`, `index`, `index_keys`, `get_keys`, `set_keys` (ключи таблиц `.get` / `.set`; сеттеры не вызываются), `get_LowerBound_keys`; результаты `GetLowerBoundValue`, `GetUpperBoundValue`, `IsEmpty`, `Contains_0410`, `LowerBound`, `LowerBound_Value`, `LowerBound_Type`, `get_LowerBound`, `get_LowerBound_Type`, `get_LowerBound_Value`, `get_UpperBound…`, `clone` (из метатаблицы), `clone_method` (`range:clone()`) в виде `ok`, `type`, `value`), `fonts[]`)), `title_cyrillic[]` (до 500: `panel`, `widget`, `class`, `typeface`, `size`, `styled` (проходил через `translateTextWidget`), `rich` (RichText, шрифт из `DefaultTextStyleOverride` / `GetDefaultTextStyle`), `font_read`, `font_src`, `count`, `text` (≤ 60 символов), `path`), `fonts[]` (`key` = `путь|typeface`, `role` `pre`/`post`, `count`, `texts_cyrillic`, `texts_cjk`, `texts_latin`, `sizes`, `widgets`, `panels`) |
