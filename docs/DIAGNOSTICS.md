...
Одна сборка для всех. Без файла флагов разработчика патч работает как обычно: в `C7.log` две строки после старта, обе пишутся в `after_main` (`[CPDDRuntimeFix] cyrillic font mode=…` и `v2.9.4-RU active hooks_installed=N`), модуль диагностики не загружается, в горячих путях `Init.lua` добавлена одна проверка `runtimeFixes.Diag ~= nil`. С файлом флагов сборка собирает данные для следующих этапов:
...
return {
    ...
    -- CyrillicFont = "cultures", -- режим кириллицы: "cultures" | "subfont" | "typeface" | "off"
...
### Флаг `CyrillicFont` (TASK-006, TASK-008)
Перекрывает релизную константу `CYRILLIC_FONT_MODE` в `Init.lua` (в v2.9.4 — `"typeface"`). Читается, только если в файле `Enabled = true`. Это не наблюдение, а смена поведения, поэтому флаг по умолчанию закомментирован.
...
- `cultures`: `[CPDDRuntimeFix] cyrillic font mode=cultures sub=<путь face> cultures=<…> previous=<прежнее> write=<ok(set)|ok(insert)|err> verify=<ok|fail> flush=<ok|missing|err> cyr=<yes|no|unknown> latin=<yes|no|unknown>` (`cyr`/`latin` — покрывает ли sub U+0410 / U+0041; `unknown`, пока диапазоны не читаются, см. пробу `.get`);
- при откате `mode=typeface reason=<…> … title_typeface=Regular` (`reason`: `no_sub`, `cultures_unreadable`, `write:<ошибка>`, `verify`, `verify_count`, `verify_no_sub`, `flush`); в релизе `mode=typeface title_typeface=Regular`.
