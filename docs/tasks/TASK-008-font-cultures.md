...
Статус: **шаги 1–5 исполнены** 2026-09-25 (v2.9.4-RU, режим по умолчанию `"typeface"`). **Проверка в игре: гипотеза не подтвердилась** — дважды `write=ok(set) verify=ok flush=ok` (`NotoSerif_Regular`, `NotoSansCJKsc_Regular`), на экране без изменений. Причину различает решающий тест [TASK-009](TASK-009-font-face-swap.md) (режим `face`, v2.9.5). Режим `cultures` оставлен в коде. Релиз не опубликован.
- **Итог (TASK-009, TASK-010):** решающий тест показал, что правка CompositeFont на кадре 0 доходит до Slate (`face` сработал), а `cultures` эффекта не дал. В v2.9.6 режим `cultures`, `CyrillicCultureSubs`, флаг `CyrillicCultureSub` и проба `.get`/`.set` у `FInt32Range` удалены.
