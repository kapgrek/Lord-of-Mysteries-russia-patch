...
Статус: **шаги 1–4 исполнены** 2026-09-25 (v2.9.6-RU, режим по умолчанию `"face"`), **ждут проверки в игре** по разделу «Проверка». Релиз не опубликован.
- Шаг 1: `Init.lua` — `FACE_SWAPS` (`Title` → `Regular`, `Title_SDF` / `Title_SDF_HeadName` → `Regular_SDF`), `entryIndexes`, `faceAt`, `writeFaces` (одна копия `CompositeFont` на все замены), `applyFace` с проверкой каждой записи и откатом всех. SDF-запись получает face, только если его путь содержит `_SDF`. Логика прогнана на заглушках (fengari, `temp/luacheck-node/face_test.lua`): три замены одной записью, пропуск при отсутствии `Regular_SDF`, откат всех записей при потере записи, при `verify` одной SDF-записи и при `flush=err`, `CyrillicTitleFace = "NotoSerif_Regular"` (SDF остаются `Regular_SDF`), неизвестное имя и отсутствие `Title` → `typeface`, `CyrillicFont = "cultures"` игнорируется, повтор в `after_main`.
- Шаг 3: запас по локальным переменным **не вырос** (VerifyPatch: 185 до и после, запас 15 до лимита 200): весь удалённый код стоял внутри блока `do … end`, а VerifyPatch считает только локальные переменные верхнего уровня. Цифра «182 из 190» в разделе «Первопричина» неточна. `Init.lua` сократился на ~350 строк.
