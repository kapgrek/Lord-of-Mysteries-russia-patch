...
- **Полная выгрузка в игре (`reference/godway_export/raw/2026-09-26_18xx`, сборка `reference/godway_export/summary.md`).** Ошибок и `limit` нет: 149 PNG (~160 МБ по несжатой оценке), 54 текстуры, 674 спрайта (2 атласа), 373 материала, 3 базовых `Material` (`MA_UI_TY_6_5zong_2Mask` — 18 MI и 343 MID, `MA_UI_TY_6_5zong_2Mask_UIMesh`, `M_UI_PanelFog`), 8 снимков раскладки по 2358 виджетов (~4 с на снимок), 27 483 события дорожки. **RetainerBox работает:** RT `TextureRenderTarget2D` 176×176 в формате 3 (`RTF_RGBA8_SRGB`), копия sRGB совпадает с прямым экспортом (разница 0), фон `liudong09` виден в `calib/`. Замечания: `alpha=unknown` (всего 8 полупрозрачных точек); `IsFullyStreamedIn` недоступен (`streamed=unknown` у всех); 9 кадров фона за 4 с одинаковые, у `dis01` и `liudong07` различаются; calib снят у 10 из 18 MI (серии есть только у MID); серии `_open` нет: у MI вкладки нет суффикса `_Animated`; индексы путей идут с пропусками (промежуточные подписи при листании), снимков 8.
