...
Статус: **шаги 1–5 исполнены** 2026-09-25 (v2.9.5-RU, режим по умолчанию `"typeface"`). **Проверка в игре (тест 1): `face` сработал** (`write=ok(set) verify=ok flush=ok applied_at=load`), верна H2; тесты 2–3 не понадобились. Разреженными остались SDF-начертания (имена над NPC, описание задания) — продолжение в [TASK-010](TASK-010-font-face-release.md) (v2.9.6: SDF-записи, `face` по умолчанию, `cultures`/`subfont`/`CyrillicFlush` удалены). Релиз не опубликован.
