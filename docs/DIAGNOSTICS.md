...
- `PerformanceMode` (`Init.lua`, блок в начале файла) опускает игровой лог до Warning, а `report()` в обычном режиме пишет через `Log.Info`. Поэтому при наличии флагов (`Loader.DevFlags`) `report()` пишет через `LuaCLogger.Warning` с тем же префиксом `[CPDDRuntimeFix]`. `PerformanceMode` не отключается. **При включённых флагах в `C7.log` видны строки `[CPDDRuntimeFix]` от `reportVerbose`**, без флагов — только строка `active hooks_installed=` и ошибки.
...
- Запись: закрытые части JSONL пишутся один раз; текущая часть и `hooks.json`/`fonts.json`/`session.json` переписываются раз в `FlushSeconds` (и через 5 с после открытия панели). Каждая запись ≤ 512 КБ. Выхода игры Lua не видит, поэтому **перед выходом постойте 30–40 с**.
...
| `absru-s<n>-session.json` | `sid`, `slot`, `version`, `started`, `last_flush`, `flags`, `dir`, `parts`, `lines`, `api` (какие API сработали: `GetParent`, `GetClass`, `Brush.ResourceObject`, `GetDesiredSize`, `GetCachedGeometry`, `IsVisible`, `GetPathName`), `gauges` (от `bootstrap.lua`), `budget` (`ticks`, `tick_ms_max`, `io_ms_max`, `flush_ms_max`, `queue_peak`, `flushes`), `dropped`, `errors`, `counters`, `pending`, `metrics` (весь `runtimeMetrics`) |
...
**Маркеры в C7.log** (Warning): `[AbsruDiag] session=<sid> slot=<n> dir=<путь> version=… flags=…`, `[AbsruDiag] flush #<k> parts=… queue=<n> tick_ms_max=<x> io_ms_max=<x> flush_ms=<x>`, `[AbsruDiag] disabled: <причина>`, `[AbsruDiag] error stage=<этап>: …` (первые 3 на этап), `[CPDDRuntimeFix] diagnostics unavailable: …` (модуль не запустился).
