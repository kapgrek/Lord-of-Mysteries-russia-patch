...
Одна сборка для всех. Без файла флагов разработчика патч работает как обычно: в `C7.log` две строки после старта, обе пишутся в `after_main` (`[CPDDRuntimeFix] cyrillic font mode=…` и `v2.9.3-RU active hooks_installed=N`), модуль диагностики не загружается, в горячих путях `Init.lua` добавлена одна проверка `runtimeFixes.Diag ~= nil`. С файлом флагов сборка собирает данные для следующих этапов:
...
Перекрывает релизную константу `CYRILLIC_FONT_MODE` в `Init.lua` (в v2.9.3 — `"typeface"`). Читается, только если в файле `Enabled = true`. Это не наблюдение, а смена поведения, поэтому флаг по умолчанию закомментирован.
...
Итог — одна строка в C7.log: `[CPDDRuntimeFix] cyrillic font mode=subfont face=<путь> write=<ok|exists|err> verify=<ok|fail> flush=<ok|missing|err>`, при откате `mode=typeface reason=<…> … title_typeface=Regular`, в релизе `mode=typeface title_typeface=Regular`. То же пишется в `fonts.json → cyrillic_font`. Режим выбирается при загрузке `Init.lua`, но строка выводится в `after_main`, прямо перед `active hooks_installed=`: `Log.Info` до инициализации логгера игры в C7.log не попадает (TASK-007).

В v2.9.3 `subfont`, скорее всего, откатывается: slua не отдаёт поля `FInt32Range` (в сессии 2026-09-25_0949 все `composite.subs[].ranges` — голые `userdata`), поэтому проверка записи (`LowerBound.Value`) не находит диапазон. Как читать диапазоны, покажет проба `range_probe` (ниже); правка `subfont` — TASK-007, шаг 4. Папку `Saved\Mods\logs\` можно удалить вручную (Lua удалять файлы не умеет).
